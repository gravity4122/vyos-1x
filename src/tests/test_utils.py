...
class TestVyOSUtils(TestCase):
    ...
    def test_is_addr_assigned_nonlocal_bind_ipv4_off_unassigned(self):
        ...
        with patch('vyos.utils.network.sysctl_read', return_value='0'), \
                patch('netifaces.interfaces', return_value=[]):
    ...
    def test_is_addr_assigned_vrf_filter(self):
        ...
        with patch('vyos.utils.network.sysctl_read', return_value='0'), \
                patch('netifaces.interfaces', return_value=['lo', 'eth0']), \
                patch('vyos.utils.network.get_interface_config',
                      side_effect=[iface_cfg_lo, iface_cfg_red]), \
                patch('vyos.utils.network.is_intf_addr_assigned',
                      return_value=True) as mock_is_assigned:
    ...
    def test_is_listen_port_wildcard_ipv4(self):
        ...
        conn = _make_conn('0.0.0.0', 443, 1234)
        with patch('psutil.net_connections', return_value=[conn]), \
             patch('psutil.Process') as MockProc:
    ...
    def test_is_listen_port_specific_address_match(self):
        ...
        conn = _make_conn('194.0.2.1', 443, 1234)
        with patch('psutil.net_connections', return_value=[conn]), \
             patch('psutil.Process') as MockProc:
            MockProc.return_value.name.return_value = 'haproxy'
            self.assertTrue(
